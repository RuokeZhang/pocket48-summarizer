...
- 时间线剪辑会先打开桌面弹窗或移动端全屏编辑器，可调整起止点、吸附字幕与邻近静音、预览所选区间，并选择中文、英文、双语字幕和右侧弹幕。
...
- 可供 libass 使用的 CJK 字体，生产默认安装 `fonts-noto-cjk`
- 阿里云私有 OSS Bucket
...
剪辑编辑器把 AI 时间线作为初始范围，允许在前后各 10 分钟的上下文中调整，并在距离字幕边界 1 秒内吸附语句；服务端只分析该边界附近约 3 秒 HLS 来寻找静音点并缓存结果。导出可关闭叠加，也可烧录中文、英文、双语字幕、右侧标准密度弹幕或其组合。
