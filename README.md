...
- 房间上麦仍是默认关闭的实验性 POC：仓库提供显式确认后的一次性
  ...
  capture-first 本地监控进程。监控状态和安全完成的 MP3 滚动分段会通过
  `/room-voice` 公开播放和下载。录音结束后，现有 SQLite Worker 会自动发现
  会话、临时合并音频、调用 DashScope ASR 和 LLM，并公开只读字幕与总结；
  同时抓取录音时间窗内的公开房间文字留言，在歌词播放器旁按时间同步展示。
  留言只保存公开昵称、文字、发送时间和相对录音时间，不保存用户 ID、等级、
  性别、VIP、实名或凭证。原始分段播放不依赖处理成功。监控不会自动重发短信
  或无人值守重新登录。
...
安全完成的 MP3 分段。结束的会话会自动进入 SQLite 持久化处理队列，公开页会
显示处理进度，并链接到同步歌词、同期公开文字留言和结构化总结。留言抓取与
ASR/总结使用独立租约和失败状态，任一处理失败都不影响原始录音播放。旧地址
`/admin/room-voice` 会重定向到该公开页面。
只有站点用户名大小写折叠后精确等于 `ruoke` 的已登录用户会看到账号维护区，
并可明确提交一次手机号发送短信，再在 10 分钟内提交手机号和验证码完成登录；
其他访客、普通用户和其他管理员均被服务端拒绝。
...
所有监控任务会分别热加载该文件，并在下一轮自动开始查询，无需重启。公开页
只显示每个目标的安全监控阶段、更新时间、动态解析后的房间 ID 和带目标归属
的采集摘要；不会显示流主机/端口、流指纹、文件路径、凭证状态、挑战状态或
其他登录技术信息。正在写入的会话不会公开最后一个滚动分段。短信请求至少
间隔 60 秒，系统不会自动发送、重发、轮询登录或自动登录。由于账号为单活
会话，生产激活会让同账号的官方手机 App 退出。

录音只保存在
`data/room-voice/<session-uuid>/segments/segment-%06d.mp3`。每个会话的
`session.json` 只记录安全 monitor ID、配置的目标成员名与成员 ID、房间 ID、
脱敏后的主机/协议/端口、流指纹、分段统计和状态，不保存原始流 URL、查询
参数或上麦参与者列表。成员 ID 用于把历史录音绑定到采集时的房间，而不是在
后台处理时猜测当前监控配置。每个任务默认每 60 秒加最多 5 秒随机抖动查询一次，MP3 使用
...
后台 Worker 按顺序把一个会话的 MP3 分段重新编码为 16 kHz 单声道临时音频，
上传到私有 OSS 并提交 DashScope。识别结果落库后会立即删除合并音频和 OSS
临时对象，再复用普通直播的热词、长文本分块和结构化总结逻辑。任务通过
`worker_id` 和租约持久化；进程重启会重新排队并复用已保存的 DashScope
任务 ID、ASR 结果和总结分块。失败不会删除原始录音，只有 `ruoke` 可以在
详情页明确重试可恢复错误。
