...
- 桌面剪辑器内置管理员专用的 Seedream AI 封面工作流：以精细时间线的
  ...
  人物、房间和画面主色，只做自然扩图与轻度修图，不再强制人物左右分栏，
  也不固定使用粉色或重绘宏大场景。模型明确禁止绘制文字；应用提供
  “贴纸高亮”“轻弧杂志”“贴条叠画面”三套本地排版，同一背景切换模板、
  修改主标题、重点文字和最多四条补充文字都不会再次调用 Seedream。可下载
  PNG、保留历史版本或付费换一版。游客和普通账号能看到已有结果，但不能
  调用生成、改字或将封面用于成片。
...
10. 管理员生成 AI 封面时，应用提取 MARK 原始帧并短暂上传私有 OSS，
    Seedream 分别生成 16:9 和 4:3 无文字横屏背景，应用本地叠字后将背景与
    最终 PNG 持久化；三种排版模板和所有可见文字均可在不调用模型的情况下
    重新合成，临时参考图在成功或失败后都会删除。
