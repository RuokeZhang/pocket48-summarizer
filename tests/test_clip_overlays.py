...
def test_ai_cover_overlay_renders_exact_safe_text(
    orientation, width, height
):
    document = build_ai_cover_overlay(
        ...
        title=r"她终于说出 {真心话}\\",
        highlight_text=r"重点 {词}\\",
        extra_text=["全场爆笑", "名场面"],
        orientation=orientation,
    )

    assert document.layout_style == "sticker_pop"
    assert document.title == r"她终于说出 {真心话}\\"
    assert document.highlight_text == r"重点 {词}\\"
    assert document.extra_text == ("全场爆笑", "名场面")
    assert r"\{真心话\}" in document.content
    assert "重点" in document.content
    assert r"\{词\}" in document.content
    assert "Style: AICoverTitle,Noto Sans CJK SC" in document.content
    assert "Style: AICoverHighlight,Noto Sans CJK SC" in document.content
    assert "全场爆笑" in document.content
    assert "名场面" in document.content
    assert "0:00:01.00" in document.content


def test_ai_cover_overlay_requires_exact_product_ratio():
    with pytest.raises(AppError, match="4:3"):
        build_ai_cover_overlay(
            width=1920,
            height=1200,
            title="比例错误",
            orientation="four_three",
        )


@pytest.mark.parametrize(
    "layout_style",
    ["sticker_pop", "editorial_arc", "banner_energy"],
)
@pytest.mark.parametrize(
    ("orientation", "width", "height"),
    [
...
def test_ai_cover_overlay_keeps_all_accepted_text(
    layout_style, orientation, width, height
):
    title = "封" * 80
    highlight = "重" * 60
    extras = [
    ...
    rendered = document.content.replace(r"\N", "")
    assert title in rendered
    assert highlight in rendered
    assert all(value in rendered for value in extras)
...
def test_ai_cover_overlay_keeps_shapes_out_of_face_safe_half(
    layout_style, orientation, width, height
):
    document = build_ai_cover_overlay(
        width=width,
        ...
        layout_style=layout_style,
        title="开场闲聊",
        highlight_text="与剧场趣事",
        extra_text=["SNH48 杨晔 LIVE", "直播切片 47:58"],
        orientation=orientation,
    )

    right_edges = []
    for line in document.content.splitlines():
        if ",AICoverShape," not in line:
            continue
        position = re.search(r"\\pos\((\d+),\d+\)", line)
        shape_width = re.search(r"l \d+ 0 b (\d+) 0", line)
        assert position is not None
        assert shape_width is not None
        right_edges.append(
            int(position.group(1)) + int(shape_width.group(1))
        )

    assert right_edges
    assert max(right_edges) <= round(width * 0.51)


def test_banner_energy_uses_contrasting_dark_title_outline():
    document = build_ai_cover_overlay(
        width=2560,
        height=1440,
    ...
    title_style = next(
        line
        for line in document.content.splitlines()
        if line.startswith("Style: AICoverTitle,")
    )
    extra_style = next(
        line
        for line in document.content.splitlines()
        if line.startswith("Style: AICoverExtra,")
    )
    assert "&H00E9F7FF" in title_style
    assert "&H00171417" in extra_style
